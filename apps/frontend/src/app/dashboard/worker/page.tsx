...
import { Zap, LogOut, TrendingUp, ArrowUpRight, X, Fingerprint, Activity, Sun, Moon } from "lucide-react";
...
function LiveBalance({ rate }: { rate: number }) {
  ...
  return (
    <div className="text-mono flex items-end gap-1 leading-none">
      ...
      <span className="text-[clamp(2.5rem,6vw,4rem)] font-extrabold text-fg tracking-tight">{Number(w).toLocaleString()}</span>
            ...
            {[1, 2, 3].map(s => (
              <div
                ...
                className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                  step > s ? "bg-emerald-500" : step === s ? "bg-indigo-600 dark:bg-indigo-400" : "bg-zinc-200 dark:bg-zinc-700"
                }`}
              />
            ))}
          </div>
          ...
          <div className="p-6">
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <button
                  onClick={() => setStep(2)}
                  className="w-full grad-brand text-white font-bold py-3 rounded-xl hover:opacity-90 transition-all shadow-md"
                >
                  Continue →
                </button>
              </div>
            ...
            {step === 2 && (
              <div className="space-y-5">
                    ...
                    {["MoneyGram", "Local Bank", "USDC"].map(a => (
                      <button
                        ...
                        className={`py-2.5 rounded-xl text-xs font-bold transition-all border ${
                          anchor === a
                            ? "grad-brand text-white border-transparent shadow-sm"
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => setStep(3)}
                    className="flex-1 grad-brand text-white font-bold py-2.5 rounded-xl hover:opacity-90 transition-all text-sm shadow-md"
                  >
                    Continue →
                  </button>
                </div>
            ...
            {step === 3 && (
              <div className="space-y-5 text-center">
                ...
                <button
                  onClick={confirm}
                  disabled={processing}
                  className="w-full grad-brand text-white font-bold py-3.5 rounded-xl hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-md"
                >
            ...
            {step === 4 && (
              <div className="text-center py-4">
                <div className="w-14 h-14 bg-emerald-500/15 rounded-full flex items-center justify-center mx-auto mb-4 border border-emerald-500/30">
                ...
                <button
                  onClick={onClose}
                  className="grad-brand text-white font-bold px-6 py-2.5 rounded-xl hover:opacity-90 transition-all text-sm shadow-md"
                >
                  Done
                </button>
              </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        >
          <div className="relative z-10">
            ...
            <p className="text-sm font-semibold text-val-green flex items-center gap-2">
            ...
            <motion.button
              ...
              className="mt-6 grad-brand text-white font-bold px-6 py-3 rounded-xl hover:opacity-95 transition-all flex items-center gap-2 shadow-md text-sm"
            >
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {streams.map((s, index) => (
              <motion.div
                ...
                className="card-base rounded-2xl p-6 border border-subtle hover:border-indigo-500/30 transition-all"
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 grad-brand rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-sm">
                      {s.employer[0]}
                    </div>
                    <div>
                ...
                <div className="h-2 rounded-full bg-zinc-200 dark:bg-zinc-700 overflow-hidden">
                  <div className="h-full bg-indigo-600 dark:bg-indigo-400 rounded-full" style={{ width: `${s.pct}%` }} />
                </div>
              </motion.div>
            ))}
          </div>
